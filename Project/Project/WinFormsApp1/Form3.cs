...
namespace WinFormsApp1
{
    ...
    {
        private string zoneTableName = "kidz_zone";

        public Form3() : this("")
        {
        }

        public Form3(string section)
        {
            InitializeComponent();
            zoneTableName = "kidz_zone";
        ...
        private void HandleProductClick(string productName, decimal price)
        {
            ...
            if (cleanProductName.EndsWith("tk", StringComparison.OrdinalIgnoreCase))
            {
                int lastSpace = cleanProductName.LastIndexOf(' ');
                if (lastSpace > 0)
                {
                    cleanProductName = cleanProductName.Substring(0, lastSpace).Trim();
                }
            }
            Product dbProduct = DatabaseHelper.GetProductFromZoneTableByName(zoneTableName, cleanProductName);
            ...
            if (dbProduct != null)
            {
                ...
                System.Diagnostics.Debug.WriteLine($"Found product in database: ID={dbProduct.Pid}, Name={dbProduct.Pname}, Section={dbProduct.Section}");
            }
            else
            {
                CartManager.AddProduct(cleanProductName, 1, price);
                System.Diagnostics.Debug.WriteLine($"Product not found in database, added to cart by name: {cleanProductName}");
            }
            MessageBox.Show($"{cleanProductName} added to cart!", "Product Added", 
        ...
        {
            if (label3 == null || string.IsNullOrWhiteSpace(label3.Text))
            {
                MessageBox.Show("Product information not available.", "Error", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                return;
            }

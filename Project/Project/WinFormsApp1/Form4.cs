...
namespace WinFormsApp1
{
    ...
    {
        private string zoneTableName = "racing_zone";

        public Form4() : this("")
        {
        }

        public Form4(string section)
        {
            InitializeComponent();
            zoneTableName = "racing_zone";
        ...
        private void HandleProductClick(string productName, decimal price)
        {
            ...
            if (cleanName.EndsWith("tk", StringComparison.OrdinalIgnoreCase))
            {
                int lastSpace = cleanName.LastIndexOf(' ');
                if (lastSpace > 0)
                    cleanName = cleanName.Substring(0, lastSpace).Trim();
            }
            Product dbProduct = DatabaseHelper.GetProductFromZoneTableByName(zoneTableName, cleanName);
            ...
            {
                CartManager.AddProduct(cleanName, 1, price);
            }
            MessageBox.Show($"{cleanName} added to cart!", "Product Added",
